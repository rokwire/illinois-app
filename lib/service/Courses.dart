...
class Courses with Service implements NotificationsListener {
  ...
  Future<List<Course>?> loadCourses({required String termId}) async {
    if (StringUtils.isNotEmpty(Config().gatewayUrl) && StringUtils.isNotEmpty(termId) && StringUtils.isNotEmpty(Auth2().uin)) {
      //TMP: Response? response = await Network().get("${Config().gatewayUrl}/courses/courselist?id=${Auth2().uin}&term=$termId", auth: Auth2(), headers: { ExternalAuthorizationHeader: Auth2().uiucToken?.accessToken });
      //TMP: String? responseString = (response?.statusCode == 200) ? response?.body : null;
