...
class _MedicineCoursesContentWidgetState extends State<MedicineCoursesContentWidget> implements NotificationsListener {
  ...
  void initState() {
    ...
    _medicineCourses = _getMedicineCourses();
    super.initState();
  }
  ...
  List<CanvasCourse>? _getMedicineCourses() {
    List<CanvasCourse>? medicineCourses;
    List<CanvasCourse>? allCourses = Canvas().courses;
    if (allCourses != null) {
      medicineCourses = <CanvasCourse>[];
      for (CanvasCourse course in allCourses) {
        int? courseAccountId = course.accountId;
        if ((courseAccountId != null) && Canvas.medicineCoursesAccountIds.contains(courseAccountId)) {
