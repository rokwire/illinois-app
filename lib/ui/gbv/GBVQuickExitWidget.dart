...
class GBVQuickExitWidget extends StatelessWidget {
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: Semantics(
              label: Localization().getStringEx('widget.sexual_misconduct.quick_exit.info_button', 'Quick exit information'),
              button: true,
              enabled: true,
              ...
              child: GestureDetector(
                onTap: () => _onQuickExitInfo(context),
                child: Tooltip(
                  message: Localization().getStringEx('widget.sexual_misconduct.quick_exit.info_button', 'Quick exit information'),
                  child: Semantics(
                    excludeSemantics: true,
                    child: Styles().images.getImage('info', excludeFromSemantics: false) ?? Container(),
                  ),
                ),
              ),
            ),
          ...
          Expanded(
            child: RichText(
              text: TextSpan(
                children: [
                  TextSpan(
                    text: Localization().getStringEx('widget.sexual_misconduct.quick_exit.privacy', 'Privacy: '),
                    style: Styles().textStyles.getTextStyle('widget.item.small.fat'),
                  ),
                  TextSpan(
                    text: Localization().getStringEx('widget.sexual_misconduct.quick_exit.app_activity', 'your app activity is not shared with others.'),
  ...
  Widget quickExitButton(BuildContext context) => Semantics(
    label: Localization().getStringEx('widget.sexual_misconduct.quick_exit.button', 'Quick exit'),
    button: true,
    enabled: true,
    onTap: () => _onQuickExit(context),
    child: GestureDetector(
      onTap: () => _onQuickExit(context),
      child: Tooltip(
        message: Localization().getStringEx('widget.sexual_misconduct.quick_exit.button', 'Quick exit'),
        child: Semantics(
          excludeSemantics: true,
          child: _quickExitIcon,
        ),
      ),
    ),
  );
  ...
  void _onQuickExitInfo(BuildContext context) {
    showDialog(context: context, builder: (_) => InfoPopup(
      ...
      infoTextWidget: Column(children: [
        Semantics(
          label: Localization().getStringEx('widget.sexual_misconduct.quick_exit.icon', 'Quick exit icon'),
          child: GBVQuickExitIcon(),
        ),
  ...
  Widget build(BuildContext context) =>
    Container(height: size, width: size,
      ...
      child: Center(
        child: Semantics(
          excludeSemantics: true,
          child: Styles().images.getImage('person-to-door', excludeFromSemantics: false, width: size / 2) ?? Container(),
        ),
      )
    );
}
