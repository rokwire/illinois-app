...
class _GroupEventDetailsPanelState extends State<GroupEventDetailPanel> with NotificationsListener{
  ...
  void initState() {
    _event = widget.event;
    Groups().loadAllAdminUserGroupsV3().then((groups) {
  ...
  Widget _eventImageHeader(){
    ...
    return Container(
      ...
      child:Stack(
        ...
        children: <Widget>[
          StringUtils.isNotEmpty(imageUrl) ?  Positioned.fill(child: AccessibleImageHolder(child: ModalImageHolder(child: WebNetworkImage(imageUrl: imageUrl, fit: BoxFit.cover, excludeFromSemantics: true, errorBuilder: ImageErrorBuilder.defaultBuilder)))) : Container(),
          CustomPaint(
  ...
  Widget _eventTitle(){
    dynamic category = ((_event?.attributes != null) && (Events2().isContentAttributeEnabled(ContentAttribute(id: 'category')))) ? _event?.attributes!['category'] : null;
