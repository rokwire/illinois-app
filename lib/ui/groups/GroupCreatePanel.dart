  ...
  Widget _buildNameField() {
    ...
    return Container(
      padding: EdgeInsets.symmetric(horizontal: 16),
        ...
        children: <Widget>[
         GroupSectionTitle(title: title, titleTextStyle: Styles().textStyles.getTextStyle('widget.title.tiny.highlight'), requiredMark: true),
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                Padding(
                  ...
                  child: Text(
                    labelTitle,
                    style: Styles().textStyles.getTextStyle("widget.title.tiny.highlight")
                  ),
                ),
                Padding(
...
/*  Widget _buildProjectSettingsLayout() {
  ...
  Widget _buildButtonsLayout() {
    return Semantics(container: true, child: Container( color: Styles().colors.background,
  ...
  String get _selectedMembersDescription {
    ...
    if (accounts != null) {
      for (Auth2PublicAccount account in accounts.values) {
        String? accountName = account.profile?.fullName
            ?? Auth2PublicAccountIdentifier.listForType(account.identifiers,
                Auth2Identifier.typeEmail).firstOrNull?.identifier
            ?? account.id;
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
          ...
          Expanded(child:
            Container(padding: EdgeInsets.only(left: 14, right: 4), child:
              Text(title, style: Styles().textStyles.getTextStyle("widget.title.light.regular.fat"),)
            ),
          ),
        ]),
      ),
