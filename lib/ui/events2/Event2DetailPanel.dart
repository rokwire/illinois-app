  ...
  List<Widget>? get _detailsInfoWidget {
    ...
    if (hasRegistration) {
        ...
        if (showSurvey) {
          if (_isAdmin) {
            description = registrationAvailable ?
              Localization().getStringEx('panel.event2.detail.survey.description.reg.att.svy.admin', 'This event requires registration. Attendance will be taken and a will be sent.') :
              Localization().getStringEx('panel.event2.detail.survey.description.reg.full.att.svy.admin', 'This event requires registration and its capacity is reached. Attendance will be taken and a will be sent.');
          }
      ...
      if (hasAttendance) {
        if (showSurvey) {
          if (_isAdmin) {
            description = Localization().getStringEx('panel.event2.detail.survey.description.att.svy.admin', 'Attendance will be taken at this event, and a will be sent.');
          }
          else switch (surveyHours) {
